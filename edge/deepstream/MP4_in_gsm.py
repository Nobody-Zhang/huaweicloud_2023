...
def frame_handler(pad, info):
    """
    Pad probe function to handle frame extraction
    """
    timestamp = info.get_buffer().pts

    # Perform frame extraction logic here
    # Determine whether to keep the frame based on timestamp

    # Example: Keep one frame per second
    frame_interval_ns = 1e9  # 1 second in nanoseconds
    global last_frame_time
    if timestamp - last_frame_time >= frame_interval_ns:
...
def main(args):
    ...
    for idx in range(len(test_video_file)):
        ...
        if not source_bin:
            logger.warning("Unable to create source bin \n")
        padname = "sink_%s" % idx
        ...
        srcpad.add_probe(Gst.PadProbeType.BUFFER, frame_handler)
        if not srcpad:
