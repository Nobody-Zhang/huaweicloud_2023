...
on_line_status = False


def get_fps_from_element(element):
    struct = element.query_caps(None).get_structure(0)
    numerator = struct.get_value("framerate").numerator
    denominator = struct.get_value("framerate").denominator
...
def main(args):
    ...
    # 用于接收pgie的输出
    buffer = make_elm_or_print_err("queue", "queue1", "Queue1")
    ...
    # 注册Pad Probe函数
    # fakesink_pad = fakesink.get_static_pad("sink")
    # fakesink_pad.add_probe(Gst.PadProbeType.BUFFER, pad_probe_cb, pipeline)
    buffer_pad = buffer.get_static_pad("src")
    buffer_pad.add_probe(Gst.PadProbeType.BUFFER, pad_probe_buf, pipeline)

    # nvvidconv_pad = nvvidconv.get_static_pad("sink")
    # nvvidconv_pad.add_probe(Gst.PadProbeType.BUFFER, pad_probe_nvvidconv, pipeline)

    # probe_id = buffer.get_request_pad("src_%u").add_probe(Gst.PadProbeType.BUFFER,
    # lambda pad, info, pipeline: handle_probe(pad, info, pipeline))
    ...
    buffer.link(fakesink)
    # buffer.link(queue2)
    # queue1.link(fakesink)
    # queue2.link(nvvidconv)
    # 设置最小时间阈值（单位：纳秒）
    # queue1.set_property('min-threshold-time', 500000000)  # 设置为0.5秒
    # queue2.set_property('min-threshold-time', 500000000)  # 设置为0.5秒
