...
def retinaface_run(weights = ROOT / "weights/mobilenet0.25_Final.pth",
                   network = "mobile0.25", # Backbone network mobile0.25 or resnet50
    ...
    net = load_model(net, weights, cpu)
    net.eval()
    print('Finished loading model!')
    # print(net)
    cudnn.benchmark = True
    device = torch.device("cpu" if cpu else "cuda:1")
    print(device)
    net = net.to(device)
...
# # show image
    ...
    #     name = "test.jpg"
    #     cv2.imwrite('output/' + f, img_raw)

class retinaface_model:
    def __init__(self):
        self.weights = ROOT / "weights/Resnet50_Final.pth"
        self.network = "resnet50"  # Backbone network mobile0.25 or resnet50
        self.cpu = False  # Use cpu inference
        self.confidence_threshold = 0.2  # confidence_threshold
        self.top_k = 5000  # top_k
        self.nms_threshold = 0.02  # nms_threshold
        self.keep_top_k = 750  # keep_top_k
        self.vis_thres = 0.4  # visualization_threshold
        self.cfg = cfg_mnet
        torch.set_grad_enabled(False)
        ...
        self.net.eval()
        # self.net = tf.saved_model.load('./models/tf_retinaface_mbv2/')
        print('Finished loading model!')
        # print(net)
...
if __name__ == '__main__':
    ...
    result = model.inference(source = "./curve/day_man_052_00_272_4.png")
    print(result)
    
    result = model.inference(source = "./curve/day_man_052_40_690_4.png")
    print(result)
    
    result = model.inference(source = "../Pytorch_Retinaface/curve/1.jpg")
    print(result)
    retinaface_run(video_path = "./curve/1.jpg")