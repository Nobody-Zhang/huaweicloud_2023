...
def yolo_run(weights=ROOT / 'mixed_s/best.xml',  # model.pt path(s)
    ...
    def divide_and_conquer(l, r):
        ...
        j = 1
        if sta_mid != 0:
            while int(mid - 0.5 * i * fps) >= l and f(int(mid - 0.5 * i * fps)) == sta_mid:
                i += 1
            while int(mid + 0.5 * j * fps) <= r and f(int(mid + 0.5 * j * fps)) == sta_mid:
                j += 1
            if i + j >= 7:  # 表示当前已经有2.5s，但是需要更进一步二分判断
                # 注意保存的是l1，r2的帧，因为这俩都判断是不可行的
                tmp.append([i + j == 7, int(mid - 0.5 * i * fps), int(mid + 0.5 * j * fps), sta_mid])
        divide_and_conquer(l, int(mid - fps * i * 0.5))
        divide_and_conquer(int(mid + fps * j * 0.5), r)
        return
    ...
    for i in tmp:
        min_t = (i[2] - i[1]) / fps - 1
        _ = b_search(i[1], i[1] + fps * 0.5, i[2] - fps * 0.5, i[2], 0.5, min_t * iou_presice_b_search, i[3], is_3=i[0])
        if _[0]:  # 表示当前出现了大于3s的
