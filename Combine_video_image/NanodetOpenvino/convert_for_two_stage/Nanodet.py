...
class NanoDet:
    ...
    def detect(self, img: np.ndarray, score_threshold: float, nms_threshold: float) -> list:
        ...
        print(f'preprocess cost: {1000*(time.time()-start_time)}ms')
        t1 = time.time()
        # Run Inference and process the result
        pred = self.compiled_model([img])[self.output_keys]
        print(f'Infer1 cost: {1000*(time.time()-t1)}ms')
        pred = np.squeeze(pred, 0)
        print(f'Infer2 cost: {1000*(time.time()-t1)}ms')
        pred = pred.flatten()
        print(f'Infer3 cost: {1000*(time.time()-t1)}ms\n')
        t2 = time.time()
        # decode outputs
        results = self.decode_infer(pred, self.center_priors, score_threshold)
        print(f'Decode cost: {1000*(time.time()-t2)}ms')
        t3 = time.time()
        # NMS
        dets = []
        for i in range(len(results)):
            ...
            for box in results[i]:
                dets.append(box)
        print(f'NMS cost: {1000*(time.time()-t3)}ms')
